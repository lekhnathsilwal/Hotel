...
<div id="insert">
	...
	<div id="insert2">
		<h1><center>Insert Receptionist</center></h1>
		...
		if(isset($_POST['submit'])){							//fetching variables of the form which travels in url
			...
			if($f_name!=='' && $l_name!=='' && $email!=='' && $confirm_password==$pswrd && $user_name!==''){
			    $password=md5($pswrd);
				...
				while($fetch=mysqli_fetch_array($result1)){
					  if($user_name==$fetch['user_name']){
						...
						header('location:insert_receptionist.php?errmsg=try again with different username');
						exit();
					}
					if($email==$fetch['email']){
						$_SESSION['msg']="the email is already exist";
						header('location:insert_receptionist.php?errmsg=the email is already exist');
						exit();
					}
				}
				//insert query of sql
				$query="insert into receptionist(f_name,m_name,l_name,address,email,contact,gender,country,user_name,user_password) values
				('$f_name','$m_name','$l_name','$address','$email','$contact','$gender','$country','$user_name','$password')";
				$result=mysqli_query($connection,$query);
				if($result){
					$_SESSION['msg']="Receptionist $user_name Succesfully Inserted";					
					header('location:insert_receptionist.php');
					exit();
				}
				else{
					$_SESSION['msg']="Can not insert the receptionist $user_name";
					header('location:insert_receptionist.php');
					exit();
				}
			}
		...
		?>
		<fieldset>
			<legend>Receptionist table </legend>
