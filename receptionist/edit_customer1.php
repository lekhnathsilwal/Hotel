...
<div id="insert">
  ...
  <div id="insert2">
    ...
    if(isset($_POST['edit'])){              //fetching variables of the form which travels in url
      $cid=$_POST['cid'];
      $user=$_POST['user'];
      $query1="select * from customer where cid='$cid' and user_name='$user'";
      ...
      while($count1=mysqli_fetch_array($result1)){
        ...
        <fieldset>
          <form action="edit_customer1.php" method="post">
            <table>
              <tr>
                <td><input hidden type="number" name="cid" id="cid" value="<?php echo $count1['cid']; ?>" required/></td>
              </tr>
              <tr>
    ...
    if(isset($_POST['update'])){ 
      $cid=$_POST['cid'];         //fetching variables of the form which travels in url
      $f_name=$_POST['f_name'];
      $m_name=$_POST['m_name'];
      $l_name=$_POST['l_name'];
      ...
      while($fetch=mysqli_fetch_array($result10)){
        ...
        if($email==$fetch['email']){
          ...
          header('location:edit_customer.php?errmsg=try again with different email');
          exit();
        }
      }
      ...
      if($f_name!=='' && $l_name!=='' && $email!=='' && contact!=='' && $gender!=='' && $confirm_password==$pswrd && $pswrd!=='' && $user_name!=='' && $address!==''){
        ...
        SET f_name='$f_name', m_name='$m_name', l_name='$l_name', address='$address', email='$email', contact='$contact', gender='$gender', country='$country',user_name='$user_name',password='$password'
        WHERE cid='$cid'";
        $result11=mysqli_query($connection,$db11);
        if($result11){
					$_SESSION['msg']="Customer $user_name Succesfully updated";					
    ...
    if(isset($_POST['delete'])){                           //fetching variables of the form which travels in url
      $cid=$_POST['cid'];
      $user=$_POST['user'];
      $db="DELETE FROM customer WHERE cid='$cid' and user_name='$user'";
      $result3=mysqli_query($connection,$db);
      if($result3){
        $_SESSION['msg']="Customer $user_name Succesfully deleted";					
        header('location:edit_customer.php');
        exit();
      }
      else{
        $_SESSION['msg']="Can not delete the customer $user_name";
        header('location:edit_customer.php');
        exit();
      }
    }
